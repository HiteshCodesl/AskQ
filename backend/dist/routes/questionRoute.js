...
questionRoute.post('/ask', AuthMiddleware, async (req, res) => {
    try {
        ...
        const userId = req.id;
        const createQuestion = await pool.query('INSERT INTO questions("title", "description", "userid") VALUES($1, $2, $3) RETURNING id, title,description, userid, created_at', [title, description, userId]);
